...
def distance_differences(routes,grids_time):
    """
    calculates the distance differences in km between the routes of the population taking into account the time required for the grid cell
    ...
    Returns->
    distance_diff: array, [distance_diff]
        array of the distance differences between the routes of the population
    """
    ...
    for route in routes:
        km_sum=0
        route_bearing=calculate_bearing(route)
        for i in range(len(route_bearing)-1):  #comprobar esto bien
            coordinates=route_bearing[i]
            bearing=coordinates[3] #sera coordiantes[2]
            grid_time=grids_time[bearing]
            km_sum=km_sum+grid_time[coordinates[0]][coordinates[1]]  

        distance_diff.append(km_sum)
